...
class MemberIdentityDisplayTests(TestCase):
    def setUp(self) -> None:
        ...
        create_role_assignment(member=self.confirmed_by, role=self.formal_role)
    ...
    def test_independent_deliberator_and_maintainer_duties_are_displayed_together(self) -> None:
        create_role_assignment(member=self.member, role=self.formal_role)
        ...
        self.assertEqual(display["membership"]["name"], "正式成员")
        self.assertEqual({item["name"] for item in display["duties"]}, {"议事者", "维护者"})
    ...
    def test_lost_formal_membership_hides_dependent_duties_but_preserves_records(self) -> None:
        formal_assignment = create_role_assignment(member=self.member, role=self.formal_role)
        ...
        revoke_role_assignment(assignment=formal_assignment, revoked_by=self.confirmed_by)

        display = member_identity_display(self.member)

        self.assertIsNone(display["membership"])
        self.assertEqual(display["duties"], [])
        self.assertEqual(display["restriction_reason"], "尚未取得当前有效的正式成员资格。")
