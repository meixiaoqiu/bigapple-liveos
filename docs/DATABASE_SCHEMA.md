...
MemberApplication stores public member applications. Real and simulation worlds submit the same fixed-world form at `/apply/`; zero-start simulation creates applications and login accounts first, and only creates candidate members after review.
...
| `availability_hours_per_week` | integer | 是 | 每周可投入小时。 |
| `capability_scores` | json | 是 | 能力自评，第一版用能力名称到分值的对象表达。 |
| `can_issue_responsibility_documents` | boolean | 是 | 个人是否可出具或承担书面责任文件。 |
| `document_authority_domains` | json | 是 | 可承担责任文件领域。 |
| `status` | enum string | 是 | `submitted`、`under_review`、`candidate`、`standby`、`rejected`、`withdrew`。 |
| `requested_member_no` | string | 否 | 期望成员编号；仿真会写入稳定候选编号。 |
| `account_user_id` | fk | 否 | 成员报名时创建的登录账号；审核为候选后绑定到 `Member.user`。 |
| `linked_member_id` | fk | 否 | 审核为候选后创建并关联的 `Member`。 |
...
提交会创建登录账号并追加 `member_application_submitted` 统一事件；审核会追加 `member_application_reviewed` 统一事件。报名账号在审核为候选成员时绑定到新建 `Member.user`；审核前该账号只用于查看自己的报名状态。
...
通用治理提案。角色任命不再是独立流程，而是 `proposal_type=role_appointment` 的一种提案。后续规则、政策、预算、项目、声明等也复用同一套 `Proposal -> ProposalVote -> ProposalExecution -> SystemEvent` 流程。
...
| `title` | string | 是 | 提案标题。 |
| `body` | text | 否 | 提案正文。 |
| `proposal_type` | enum string | 是 | `role_appointment`、`role_revocation`、`rule`、`policy`、`budget`、`project`、`statement`。 |
...
| `pass_ratio` | integer | 是 | 通过所需赞成比例，1 到 100。 |
...
| `action_type` | enum string | 是 | `create_role_assignment`、`revoke_role_assignment`、`create_rule`、`create_policy`、`record_statement`、`manual`。 |
...
第一版重点支持 `role_appointment` 提案通过后的 `create_role_assignment` 执行，并保持幂等：同一个已执行提案不会重复创建 active `RoleAssignment`。
