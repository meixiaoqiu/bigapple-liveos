...
MemberApplication stores public member applications. Real and simulation worlds submit the same fixed-world form at `/apply/member/`; zero-start simulation creates applications first, and only creates candidate members after review.
...
提交会追加 `member_application_submitted` 统一事件；审核会追加 `member_application_reviewed` 统一事件。当前报名阶段不强制创建登录账号，避免破坏现有 `User.username == Member.member_no` 的成员身份推导；账号绑定在接纳为成员后处理。
