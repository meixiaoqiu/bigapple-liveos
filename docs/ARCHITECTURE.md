...
`django_session` is a control-table convenience; world users are not stored in control. `auth_user` is migrated into each world database so real and simulation worlds use the same login and authorization logic. The world login form writes the selected `world_id` into session state, and middleware prevents that session from being reused across a different world.
