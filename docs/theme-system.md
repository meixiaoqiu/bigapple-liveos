...
```python
{
    "hero": {"title": "大苹果观察台", "subtitle": "", "status_label": "", "status_level": "watch"},
    "stats": [],
    "missions": [],
    "events": [],
    "map_points": [],
...
`GET /observer/?theme=<theme_key>` 只用于人工验证当前请求的临时主题预览，不写入 session，也不修改数据库。需要持久切换主题时必须使用上面的 POST 入口。
