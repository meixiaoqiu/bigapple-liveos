...
```powershell
.\scripts\ai\codebuddy_make_patch.ps1 -Requirement "本次需求描述" -ContextPath core,simulation_lab
...
`codebuddy_make_patch.ps1` 只允许 CodeBuddy 输出 unified diff 到 `var/ai_runs/<run-id>/proposal.patch`，不直接改工作区。`codebuddy_apply_patch_guard.ps1` 会检查 patch 是否只触碰允许的文本文件、是否避开 `.env`、密钥、迁移、`start.bat`、生成目录和其他危险路径，并先运行 `git apply --check`。只有 Codex 或人工审阅通过后，才允许加 `-Apply`。

这些脚本通过管道把受限上下文传给 `codebuddy`，调用时禁用 CodeBuddy 内置工具，并会截断长输入、脱敏常见 secret、避免把 `.env`、密钥、数据库密码或私密配置传给 CodeBuddy。CodeBuddy 输出不能直接作为最终事实：涉及模型、迁移、权限、统一事件账本、真实/仿真数据库边界、危险后台操作时，必须由 Codex 或人工重新审查后再应用。
...
- `scripts/ai/codebuddy_make_patch.ps1 -Requirement "..." -ContextPath path1,path2`：生成待审 unified diff，不直接修改工作区。
- `scripts/ai/codebuddy_apply_patch_guard.ps1 -PatchFile path\to\proposal.patch`：校验待审 patch；显式加 `-Apply` 后才应用。

统一 runner 会生成 `var/ai_runs/<run-id>/prompt.txt`、`stdout.txt`、`stderr.txt`、`metadata.json` 和 `report.md`，并估算输入/输出 token。patch 工作流额外生成 `proposal.patch`。`var/ai_runs/` 不进入 Git。CodeBuddy 可以承担轻中度草稿工作，但所有 patch 必须通过 guard、测试和 Codex/人工审查。
