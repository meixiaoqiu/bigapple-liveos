...
class BootstrapWorldCommandTests(TestCase):
    def call_bootstrap(self, **options) -> str:
        ...
        defaults = {
            ...
            "world_administrator_password": "world-test-password",
            "stdout": output,
        }
    ...
    def administrator_role(self) -> Role:
        organization = Organization.objects.get(name=ROLE_CATALOG_ORGANIZATION_NAME)
        return Role.objects.get(organization=organization, name=ROLE_ADMINISTRATOR)

    def test_bootstrap_creates_control_admin_and_world_administrator(self) -> None:
        self.call_bootstrap()

        user_model = get_user_model()
        control_user = user_model.objects.get(username="wzy")
        world_user = user_model.objects.get(username="member-administrator-0001")
        member = Member.objects.get(member_no="member-administrator-0001")
        ...
        self.assertTrue(user_has_permission(world_user, ADMINISTRATION_VIEW_ADMIN_PERMISSION))

    def test_bootstrap_is_idempotent_for_world_administrator(self) -> None:
        self.call_bootstrap()
        member = Member.objects.get(member_no="member-administrator-0001")
        role = self.administrator_role()
        ...
        self.assertEqual(get_user_model().objects.filter(username="member-administrator-0001").count(), 1)
        self.assertEqual(Member.objects.filter(member_no="member-administrator-0001").count(), 1)
        self.assertEqual(
    ...
    def test_new_world_administrator_requires_password(self) -> None:
        with self.assertRaises(CommandError) as captured:
            self.call_bootstrap(skip_control_admin=True, world_administrator_password="")

        self.assertIn("world administrator requires", str(captured.exception))

    def test_bootstrap_requires_at_least_one_target(self) -> None:
        with self.assertRaises(CommandError) as captured:
            self.call_bootstrap(skip_control_admin=True, skip_world_administrator=True)
