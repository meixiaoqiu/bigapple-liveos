...
- **THEN** 当前仍存在的导航分组、事务分组和待处理事项继续按单列竖版顺序显示

### Requirement: 宽度改造初次交付不改变现有功能
系统 SHALL 在竖版外壳初次交付时保留宽度改造前 Workspace 首页的全部现有模块、入口、文案顺序和权限条件。初次交付完成后，经使用者后续明确确认的独立增量 MAY 删除或迁移模块；当前页面事实以已实施的后续规格为准。

#### Scenario: 成员访问改造后的 Workspace
- **WHEN** 任意具备 Workspace 访问资格的成员打开首页
- **THEN** 初次宽度改造不删除其原先有权看到的模块和入口；后续已确认删除或迁移的模块不受永久存在约束
- **THEN** 其原先无权看到的模块和入口不会因布局改造而出现

#### Scenario: 后续增量清理首页模块
- **WHEN** 竖版外壳已经完成验收，且后续规格明确删除或迁移某个首页模块
- **THEN** 系统可实施该后续决定而不与初次宽度改造的保留基线冲突
- **THEN** 最大宽度、单列画布和权限边界继续保持
