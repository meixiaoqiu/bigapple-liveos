...
- 将共享运行时品牌文案统一为“大苹果社区”，品牌首页目标地址保持不变；页头左侧读取该共享品牌，右侧改为一个有明确可访问名称的菜单入口。
...
- 非目标：不重新设计其他 Workspace 子页面、Observer、财务页、事件页或公共运行时页头；除统一 `brand_label` 文案外，不修改 `runtime_nav` 的导航构建、权限、顺序或请求方法。
...
- 将 `live_os/runtime_nav.py` 提供的共享运行时品牌文案统一为“大苹果社区”；使用共享运行时页头的页面会同步显示该品牌，但其页头结构和导航行为不变。
