...
- 除 `brand_label` 文案外，不修改 `runtime_nav` 的导航项目构建、权限、顺序、URL 或请求方法。
...
`runtime_nav.brand_label` 统一使用用户确认的产品名称“大苹果社区”。Workspace 紧凑页头和既有共享运行时页头都继续读取同一上下文，因此无需在模板中重复品牌文案；共享页头的结构、导航项目和权限逻辑均保持不变。
